...
렌더링하지 않으므로 서식 기호 없이, 프로젝트마다 같은 순서의 라벨 블록으로 낸다.
  ...
  [핵심 성과]  「- 」 한 줄에 하나. 하위 지표는 2칸 들여쓴 「- 」
  [주요 기여]  1. 2. 3. 번호. 세부는 3칸 들여쓴 「- 」
...
def plain_lines(block):
    """[핵심 성과]용 — 상위는 「- 」, 하위는 2칸 들여쓴 「- 」. 플랫폼 칸은 들여쓰기만으론 층이 안 보여 기호를 붙인다."""
    return [('  - ' if d else '- ') + t for d, t in items_of(block)]


def numbered_lines(block):
    """[주요 기여]용 — 상위는 1. 2. 3., 하위는 번호 폭에 맞춰 3칸 들여쓴 「- 」. 항목 사이는 빈 줄."""
    o, n = [], 0
